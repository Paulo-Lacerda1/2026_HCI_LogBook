...
function swipeSuggestion(type) {
  ...
  // Se o utilizador gostar (Like), guardamos temporariamente o destino
  if (type === 'like') {
    trip.voteResults.destination = suggestion.city;
  }

  // Avançamos sempre para a próxima carta
  trip.currentSuggestionIndex = index + 1;

  // VERIFICAÇÃO: Chegámos ao fim das 3 sugestões?
  if (trip.currentSuggestionIndex >= trip.suggestions.length) {
    
    // Se houve algum "Like" durante a votação
    if (trip.voteResults.destination !== 'TBD') {
      
      const ok = confirm(`Destino Escolhido: ${trip.voteResults.destination}. Tem a certeza?`);
      
      if (ok) {
        // FINALIZAR: Se clicar em Sim, bloqueia as decisões e gera o roteiro
        ...
        trip.itinerary = [{
          ...
          items: [`✅ Destination confirmed: ${trip.voteResults.destination}`, `Next step: Vote on accommodation.`]
        }];
        trip.pendingActions = trip.pendingActions.filter(a => !a.description.includes('vote'));
        trip.missingItem = '';
      } else {
        // RECOMEÇAR: Se clicar em Não, limpa tudo e volta à primeira carta
        trip.voteResults.destination = 'TBD';
...
function closeCurrentTrip() {
  ...
  // Prevenção de erros: Verifica se já está fechada
  if (trip.status === 'closed') {
    alert('This trip is already closed and archived.');
    return;
  }

  // Pede confirmação ao utilizador
  const confirmClose = confirm(`Do you want to complete "${trip.name}"? \n\nThis will mark the trip as Closed and move it to your History tab.`);
  
  if (confirmClose) {
    // Muda o estado da viagem
    trip.status = 'closed';
    
    // Atualiza o itinerário para refletir o fim da viagem
    trip.itinerary = [{ 
      day: 'Final', 
      nowNextTitle: 'Trip completed', 
      items: [
        '✅ Trip archived successfully.', 
        'Check your final balances in the Expenses tab.'
      ] 
    }];
    
    // Limpa ações pendentes, se existirem
    trip.pendingActions = [];
    
    saveTripsToStorage();
    
    // Muda a aba atual para o "Histórico" e volta ao menu de viagens
